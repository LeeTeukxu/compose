...
//	 点击登录时手机号为空的弹框
	...
	$('.mlbtn1').click(function() {
		...
		var pwd = $('.f-ml-put2').val();
		var code = $('.f-ml-put3').val();
		if(val == '') {
			$('.f-ml-alternull').show();
			setTimeout(function(){
				$('.f-ml-alternull').hide();
		...
		if (isSave = true){
			var arg = {username:val, password:pwd, piccode:code};
			$.post('/doLogin',arg,function(rs){
				if(rs.success){
...
$('.f-ml-put1').blur(function() {
		var passw = /^1[3|4|5|7|8]\d{9}$/;
		if(!$('.f-ml-put1').val()) {
			$('.success').removeClass('f-icons-on');
			$('.f-ml-phone-warn').show();
			$('.f-ml-phone-warn').html('<div class="f-icons-phone l"' + '></div>' + '请输入帐号!');
			isSave = false;
		}else if (!passw.test($('.f-ml-put1').val())) {
			$('.success').removeClass('f-icons-on');
			$('.f-ml-phone-warn').html('<div class="f-icons-phone l"' + '></div>' + '请重新输入正确的手机号码!');
			$('.f-ml-phone-warn').show();
			isSave = false;
		}else {
...
//	密码验证
	$('.f-ml-put2').blur(function() {
		...
		if(!$('.f-ml-put2').val()) {
			...
			$('.successpw').removeClass('f-icons-on2');
			$('.f-ml-pass-warn').html('<div class="f-icons-pass l"' + '></div>' + '请输入6位到12位之间的密码!');
			$('.f-ml-pass-warn').show();
			isSave = false;
		}else {
