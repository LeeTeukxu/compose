...
dragSort.prototype = {
    init: function init() {
            ...
            if (_this.hasClassName(target, dragClass)) {
                ...
                if (targetIndex != dragingIndex) { // 拖拽框位置判断
                    ...
                    for (var i = 0; i < animateObj.length; i++) {
                        ...
                        var index = i;
                        index++;
                        if (index == animateObj.length){
                            var a = "";
                        //     var listItem = [];
                        //     var listAttID = [];
                        //     var x = 0;
                        //     var iid = mini.loading('正在保存排序.....');
                        //     $("#drag-div").find("div").each(function (index,element) {
                        //         if ($(this).attr('class') == "drag-item") {
                        //             x++;
                        //             listItem.push(x);
                        //             listAttID.push(element.id);
                        //         }
                        //     });
        ...
        node.ondragend = function(event) {
            ...
            var url = "/DrawingSpecification/dragSort";
            var json = {"SortItem": listItem, "SortAttID": listAttID, "Type": atttype, "RowValue": rowValue};
            $.ajax({
                contentType: 'application/json',
                method: 'POST',
                url: url,
