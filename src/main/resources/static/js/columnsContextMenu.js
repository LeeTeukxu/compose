    ...
    onMenuItemClick: function (e) {
        ...
        if(item.name=="saveConfigShare"){
            function g(){
                ...
                var result=getter.getAll();
                result.url=decodeUrl(window.location.href);
                $.post('/gridConfig/save',{Data:mini.encode(result),Share:1},function(r){
                    if(r.success){
                        mini.alert('当前表格显示设置保存成功，刷新后可以查看最新设置结果!','系统提示');
                    } else mini.alert(r.message || "保存失败，请稍候重试！");
                });
    ...
    this.doApply=function(config){
        ...
        if(visibleObj){
            ...
            for(var i=0;i<columns.length;i++){
                ...
                var field=column["field"];
                if(field){
                    for(var field in indexObj){
                        var index=parseInt(indexObj[field]);
                        if(index==i){
                            var findColumn=grid.getColumnByField(field);
                            if(findColumn!=null){
                                cs.push(findColumn);
                                break;
                            }
            ...
            if(cs.length==columns.length)grid.setColumns(cs);
            grid.doLayout();
