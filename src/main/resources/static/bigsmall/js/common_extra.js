...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	var showloading = function (zimg, pn) {
		...
		//(document.body.clientWidth - 42) / 2 + 'px'
	...
	var adjust = function(e, a) {
		...
		if(!a) {
			...
			if(imgw > imgh) {
				...
				ele.height += step;
				ele.width = imgw*((ele.height + step)/imgh);
			} else if(imgw < imgh) {
				ratio = step/ele.width;
				ele.width += step;
				ele.height = imgh*((ele.width + step)/imgw);
			}
			if(BROWSER.ie && BROWSER.ie<7) {
				setMenuPosition('', menuid, '00');
			} else {
				...
				var imgX = clientX - menuleft - 900;
				var newTop = (menutop - imgY*ratio) + 'px';
				//(menuleft - imgX*ratio) + 'px'
				var newLeft =(menuleft - imgX*ratio) + 'px';
				menu.style.top = newTop;// newTop
				menu.style.left = newLeft;//newLeft
		...
		menu.style.width = (parseInt(ele.width < 300 ? 300 : parseInt(ele.width + step)) + 20) + 'px';
		var mheight = (parseInt(ele.height + step) + 50);
		menu.style.height = mheight + 'px';

		document.getElementById(menuid + '_zoomlayer').style.height = (mheight < 120 ? 120 : mheight) + 'px';
		adjustpn(ele.height + step);
		doane(e);
	};
	if(!menu && !pn) {
...
function _zoom_page(paid, showexif) {
	var imagesrc = document.getElementById('aimg_' + paid).getAttribute('zoomfile') ? document.getElementById('aimg_' + paid).getAttribute('zoomfile') : document.getElementById('aimg_' + paid).getAttribute('file');
	zoom(document.getElementById('aimg_' + paid), imagesrc, 0, 1, showexif ? 1 : 0);
}
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	setTimeout(function () {hideMenu(1, 'prompt');document.getElementById('append_parent').removeChild($('ntcwin'));}, 1500);
}
